module.exports = (shepherd) => {
  ...
  shepherd.get('/electrum/decoderawtx', (req, res, next) => {
    if (shepherd.checkToken(req.query.token)) {
      ...
      shepherd.log('electrum decoderawtx input tx ==>', 'spv.decoderawtx');
      ...
      if (req.query.parseonly ||
        ...
        shepherd.log(decodedTx.inputs[0], 'spv.decoderawtx');
        shepherd.log(decodedTx.inputs[0].txid, 'spv.decoderawtx');

        ecl.connect();
        ecl.blockchainTransactionGet(decodedTx.inputs[0].txid)
        .then((json) => {
          ecl.close();
          shepherd.log(json, 'spv.decoderawtx');
