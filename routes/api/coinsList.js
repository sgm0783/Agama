...
module.exports = (api) => {
  api.loadCoinsListFromFile = () => {
    try {
      if (fs.existsSync(`${api.agamaDir}/api/coinslist.json`)) {
        const _coinsList = JSON.parse(fs.readFileSync(`${api.agamaDir}/api/coinslist.json`, 'utf8'));
  ...
  api.get('/coinslist', (req, res, next) => {
    if (api.checkToken(req.query.token)) {
      if (fs.existsSync(`${api.agamaDir}/api/coinslist.json`)) {
        fs.readFile(`${api.agamaDir}/api/coinslist.json`, 'utf8', (err, data) => {
          if (err) {
            const retObj = {
              msg: 'error',
              result: err,
  ...
  api.post('/coinslist', (req, res, next) => {
    if (api.checkToken(req.body.token)) {
      ...
      if (!_payload) {
        ...
        fs.writeFile(`${api.agamaDir}/api/coinslist.json`, JSON.stringify(_payload), (err) => {
          if (err) {
            const retObj = {
              msg: 'error',
              result: err,
